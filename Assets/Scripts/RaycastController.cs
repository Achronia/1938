...
public class RaycastController : MonoBehaviour
{
    ...
    private void RaycastControll()
    {
        ...
        if (Physics.Raycast(ray,out hit ,_raycastDistance))
        {
            ...
            {
                _uýController = true;
                // e tuþuna basýldýðýnda ve karþýdaki obje IInteractable koduna sahipse IInteractable kodunu çalýþtýrýyor artýk o nereye baðlýysa
                ...
                if (_uýController)
                {
                    _buttonUI.gameObject.SetActive(true);
                }
            }
        }
    ...
    private void Delayer()
    {
        if (!_uýController)
        {
            _buttonUI.gameObject.SetActive(false);
        }
    }
